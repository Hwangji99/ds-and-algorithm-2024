...
def insert_data(pos, friend):
    ...
    # 정상적인 처리 시작
    kakaotalk.append(None) # 제일 뒤에 빈칸 추가
...
# 데이터 삭제함수
def delete_data(pos): # 데이터 삭제 시 위치값만
    if pos < 0 or pos >= len(kakaotalk): # 책에는 > 되어있는데 틀린 것이다 >=로 변경해야 함
        print('데이터 삭제범위 초과')
        return
    ...
    del(kakaotalk[size-1]) # 배열의 맨 마지막 값은 삭제
