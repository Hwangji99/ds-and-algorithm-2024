...
# start부터 시작해서 끝까지 node.data 출력
def printNodes(start):
    # global curr # 해도 되고 안해도 되고
...
def insertNode(find, data):
    ...
    while curr.link != None: # 중간에 노드 삽입
        ...
        if curr.data == find: # 데이터를 찾았으면
            ...
            prev.link = node # 이전 노드 뒤에 새 노드를 연결
            return # 함수 탈출
...
def findNode(find):
    ...
    while curr.link != None:
        ...
        if curr.data == find:
            return curr
    return Node() # 위에 만족 못하면 빈노드 리턴 함수 탈출
...
if __name__ == '__main__':
    ...
    node = Node()
    node.data = originData[0] # = '찬규'
    head = node # head는 node를 가리킴

    for name in originData[1:]: # 1번 인덱스부터 끝까지 반복
        prev = node
        node = Node()
        node.data = name
        prev.link = node
    ...
    insertNode('소민', '나연')
    printNodes(head)
    # 정환 -> 찬규 -> 지환 -> 경민 -> 수진 -> 소민 -> 은수

    insertNode('현지', '재린') # '현지'란 노드가 없으니 마지막 삽입
    print('마지막 노드 삽입')
    printNodes(head)
    # 정환 -> 찬규 -> 지환 -> 경민 -> 수진 -> 소민 -> 은수 -> 재린

    # 노드 삭제
    deleteNode('정환')
    print('맨 앞 노드 삭제')
    printNodes(head)
    # 찬규 -> 지환 -> 경민 -> 수진 -> 소민 -> 은수 -> 재린
